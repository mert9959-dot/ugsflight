...
def get_serpapi_price():
    """Google Flights'tan SerpAPI aracılığıyla gerçek fiyat çeker."""
    try:
        params = {
...
def get_simulated_price():
    """SerpAPI çalışmazsa yedek simülasyon."""
    import random
    havayollari = {"THY": 2800, "Pegasus": 2400, "SunExpress": 2300}
    sonuclar = {
...
def fiyat_mesaji_olustur(veri: dict, onceki_fiyat: int | None) -> tuple[str, str]:
    ...
    mesaj = (
        ...
        f"{hedef_uyari}\n\n"
        f"Tüm fiyatlar:\n{tum_fiyatlar_str}\n\n"
        f"🕐 {datetime.now().strftime('%H:%M')} | {kaynak_str}"
        f"{IMZA}"
    )
...
async def dur(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for job in ctx.job_queue.get_jobs_by_name(f"takip_{kullanici_id}"):
...
async def kontrol_et(ctx: ContextTypes.DEFAULT_TYPE, kullanici_id: int):
    ...
    try:
        ...
        if tur in ("dusus", "hedef") or onceki is None:
            await ctx.bot.send_message(kullanici_id, mesaj, parse_mode="Markdown")
            log.info(f"Bildirim gönderildi → {kullanici_id} | {fiyat:,} TL | {tur}")
        else:
            log.info(f"Kontrol tamam → {fiyat:,} TL ({tur}) | bildirim yok")
...
def main():
    ...
    log.info("✈️ Uçuş fiyat takip botu başlatıldı! (3 saatte bir kontrol)")
